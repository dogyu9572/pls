...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (showPickerBtn) {
        showPickerBtn.addEventListener('click', function(e) {
            ...
            if (iconPickerContainer.style.display === 'block') {
                ...
                adjustIconPickerPosition();
            }
        });
    }

    // 아이콘 선택기 위치 조정 함수
    function adjustIconPickerPosition() {
        if (!iconPickerContainer || !showPickerBtn) return;

        const container = iconPickerContainer;
        const button = showPickerBtn;
        const buttonRect = button.getBoundingClientRect();
        const containerRect = container.getBoundingClientRect();
        
        // 기존 위치 클래스 제거
        container.classList.remove('show-above', 'show-left');
        
        // 화면 하단 공간 확인
        const spaceBelow = window.innerHeight - buttonRect.bottom;
        const containerHeight = 350; // 검색창 + 아이콘 그리드 + 패딩
        
        // 아래쪽 공간이 부족하면 위쪽에 표시
        if (spaceBelow < containerHeight) {
            container.classList.add('show-above');
        }
        
        // 오른쪽 공간 확인
        const spaceRight = window.innerWidth - buttonRect.left;
        const containerWidth = 330; // 컨테이너 너비 + 패딩
    ...
    window.addEventListener('resize', function() {
        if (iconPickerContainer && iconPickerContainer.style.display === 'block') {
            adjustIconPickerPosition();
        }
    });
