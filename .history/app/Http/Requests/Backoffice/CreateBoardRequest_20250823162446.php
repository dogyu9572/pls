...
class CreateBoardRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'custom_fields.*.type' => 'required|in:text,select,checkbox,radio,date',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // 커스텀 필드의 max_length 유효성 검사 (한글 기준)
            if ($this->has('custom_fields') && is_array($this->custom_fields)) {
                foreach ($this->custom_fields as $index => $field) {
                    if (isset($field['max_length']) && !empty($field['max_length']) && 
                        in_array($field['type'], ['text']) && 
                        isset($field['name']) && !empty($field['name'])) {
                        
                        $fieldName = $field['name'];
                        $maxLength = (int)$field['max_length'];
                        $inputValue = $this->input("custom_field_{$fieldName}");
                        
                        if (!empty($inputValue)) {
                            // 한글 글자 수 계산 (한글 1글자 = 3바이트, 영문 1글자 = 1바이트)
                            $charCount = 0;
                            $length = strlen($inputValue);
                            
                            for ($i = 0; $i < $length; $i++) {
                                $char = ord($inputValue[$i]);
                                if ($char >= 0x80) {
                                    // 한글이나 다국어 문자 (3바이트)
                                    $charCount++;
                            ...
                            if ($charCount > $maxLength) {
                                $validator->errors()->add("custom_field_{$fieldName}", "{$field['label']}은(는) 최대 {$maxLength}자(한글 기준)까지 입력 가능합니다. 현재 {$charCount}자입니다.");
                            }
                        }
                    }
                }
