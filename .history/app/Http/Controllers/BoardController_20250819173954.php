...
class BoardController extends Controller
{
    ...
    public function index(Request $request, $slug)
    {
        ...
        // 스킨 정보 가져오기
        $skin = $board->skin;
        $skinPath = "boards.skins.{$skin->directory}.list";
    ...
    public function show($slug, $id)
    {
        ...
        $skin = $board->skin;
        $skinPath = "boards.skins.{$skin->directory}.view";
    ...
    public function update(UpdateBoardPostRequest $request, $slug, $id)
    {
        ...
        // 비밀글 설정
        if ($request->input('is_secret') !== null) {
            $data['is_secret'] = (bool) $request->input('is_secret');
        }

        // 공지 설정 (관리자만 가능)
        if (Auth::check() && Auth::user()->isAdmin() && $request->input('is_notice') !== null) {
            $data['is_notice'] = (bool) $request->input('is_notice');
        }
    ...
    public function checkPassword(CheckPasswordRequest $request, $slug, $postId, $action)
    {
        ...
        if ($this->boardService->verifyPassword($request->password, $post->password)) {
            if ($action === 'edit') {
    ...
    public function checkCommentPassword(CheckPasswordRequest $request, $slug, $postId, $commentId)
    {
        ...
        if ($this->boardService->verifyPassword($request->password, $comment->password)) {
