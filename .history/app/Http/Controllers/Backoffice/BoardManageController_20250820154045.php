...
class BoardManageController extends BaseController
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'table_name' => 'nullable|alpha_dash|max:50',
        ...
        // table_name 중복 체크 (삭제된 데이터 제외)
        if (!empty($request->table_name)) {
            if (!Board::isTableNameAvailable($request->table_name)) {
                $validator->errors()->add('table_name', '이미 사용 중인 table_name입니다.');
            }
        }
        ...
        // table_name 자동 생성 (비어있거나 중복일 경우)
        $data = $request->all();
        if (empty($data['table_name'])) {
            $data['table_name'] = $this->generateUniqueTableName($data['name']);
        } else {
            // table_name이 중복일 경우 자동으로 수정 (삭제된 데이터 제외)
            $originalTableName = $data['table_name'];
            $counter = 1;
            while (!Board::isTableNameAvailable($data['table_name'])) {
                $data['table_name'] = $originalTableName . '-' . $counter;
                $counter++;
            }
        }
