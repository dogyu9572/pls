        ...
        try {
            ...
            if ($request->hasFile('logo')) {
                ...
                try {
                    ...
                    \Log::info('로고 파일 업로드 성공', [
                        'stored_path' => $logoPath,
                        'public_url' => $data['logo_path']
                    ]);
                } catch (\Exception $e) {
                    \Log::error('로고 파일 업로드 실패', [
                        'error' => $e->getMessage(),
                        'trace' => $e->getTraceAsString()
                    ]);
                }
            } else {
                \Log::info('로고 파일이 첨부되지 않음');
            }

            // 파비콘 파일 업로드 처리
            \Log::info('파비콘 파일 확인', ['hasFile' => $request->hasFile('favicon')]);
            ...
            if ($request->hasFile('favicon')) {
                ...
                try {
                    ...
                    \Log::info('파비콘 파일 업로드 성공', [
                        'stored_path' => $faviconPath,
                        'public_url' => $data['favicon_path']
                    ]);
                } catch (\Exception $e) {
                    \Log::error('파비콘 파일 업로드 실패', [
                        'error' => $e->getMessage(),
                        'trace' => $e->getTraceAsString()
                    ]);
                }
            } else {
                \Log::info('파비콘 파일이 첨부되지 않음');
            }
