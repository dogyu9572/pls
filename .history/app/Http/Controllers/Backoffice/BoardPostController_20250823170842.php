...
class BoardPostController extends Controller
{
    ...
    public function store(Request $request, $slug)
    {
        ...
        if ($request->hasFile('thumbnail')) {
            $thumbnailPath = $request->file('thumbnail')->store('thumbnails/' . $slug, 'public');
            ...
            foreach ($board->custom_fields_config as $fieldConfig) {
                ...
                if ($fieldConfig['required']) {
                    if ($fieldConfig['type'] === 'checkbox') {
                        // 체크박스는 배열이어야 하고 최소 1개 이상 선택되어야 함
                        if (empty($fieldValue) || !is_array($fieldValue) || count($fieldValue) === 0) {
                            return redirect()->back()
                                ->withErrors(["custom_field_{$fieldName}" => "{$fieldConfig['label']}은(는) 최소 1개 이상 선택해야 합니다."])
                                ->withInput();
                        }
                    } else {
                        // 다른 타입은 기존 로직
                        if (empty($fieldValue)) {
                            return redirect()->back()
                                ->withErrors(["custom_field_{$fieldName}" => "{$fieldConfig['label']}은(는) 필수 입력 항목입니다."])
                                ->withInput();
                        }
                    }
                }
                ...
                // 최대 길이 검증
                if ($fieldConfig['max_length'] && strlen($fieldValue) > $fieldConfig['max_length']) {
                    return redirect()->back()
                        ->withErrors(["custom_field_{$fieldName}" => "{$fieldConfig['label']}은(는) 최대 {$fieldConfig['max_length']}자까지 입력 가능합니다."])
                        ->withInput();
                }
    ...
    public function update(Request $request, $slug, $postId)
    {
        ...
        // 썸나일 이미지 처리
        $thumbnail = $post->thumbnail; // 기존 썸나일 유지
        ...
        if ($request->has('existing_attachments')) {
            foreach ($request->input('existing_attachments') as $existingAttachment) {
                if (is_string($existingAttachment)) {
                    $attachment = json_decode($existingAttachment, true);
                    if ($attachment && is_array($attachment)) {
                        $attachments[] = $attachment;
                    }
                } elseif (is_array($existingAttachment)) {
