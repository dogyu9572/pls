...
class BoardSkinController extends BaseController
{
    ...
    {
        $skins = BoardSkin::orderBy('created_at', 'desc')->get();
    ...
    {
        $boardSkin = BoardSkinService::createBoardSkin($request->validated());

        return redirect()->route('backoffice.board-skins.index')
            ->with('success', '스킨이 성공적으로 생성되었습니다.');
    }
    ...
    {
        BoardSkinService::updateBoardSkin($boardSkin, $request->validated());

        return redirect()->route('backoffice.board-skins.index')
            ->with('success', '스킨이 성공적으로 수정되었습니다.');
    }
    ...
    {
        BoardSkinService::deleteBoardSkin($boardSkin);

        return redirect()->route('backoffice.board-skins.index')
            ->with('success', '스킨이 성공적으로 삭제되었습니다.');
    }

    /**
     * 스킨 템플릿 편집기를 표시합니다.
     */
    public function editTemplate(BoardSkin $boardSkin, Request $request)
    {
        $template = $request->get('template', 'list');
        $content = BoardSkinService::getTemplateContent($boardSkin, $template);
    ...
    public function updateTemplate(Request $request, BoardSkin $boardSkin)
    {
        ...
        BoardSkinService::updateTemplateContent($boardSkin, $template, $content);
