...
class BoardFileGeneratorService
{
    ...
    private function createBoardMigration(Board $board): void
    {
        ...
        $fileName = "{$timestamp}_create_board_posts_{$board->slug}_table.php";
...
@section('title', '{$board->name} - 게시글 상세')
...
return new class extends Migration
{
    ...
    {
        Schema::create('board_posts_{$board->slug}', function (Blueprint \$table) {
            \$table->id();
            \$table->string('title');
            \$table->text('content');
            \$table->string('author_name');
    ...
    {
        Schema::dropIfExists('board_posts_{$board->slug}');
    }
};";
    }
}
