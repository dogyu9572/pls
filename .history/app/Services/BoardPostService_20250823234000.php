...
class BoardPostService
{
    ...
    {
        $data = $this->preparePostData($validated, $request, $slug);
     ...
     * 게시글 데이터 준비
     */
    private function preparePostData(array $validated, Request $request, string $slug): array
    {
        return [
            'user_id' => null,
            'author_name' => '관리자',
    ...
    {
        $data = $this->prepareUpdateData($validated, $request, $slug);
     ...
     * 수정 데이터 준비
     */
    private function prepareUpdateData(array $validated, Request $request, string $slug): array
    {
        return [
            'title' => $validated['title'],
            'content' => $this->sanitizeContent($validated['content']),
