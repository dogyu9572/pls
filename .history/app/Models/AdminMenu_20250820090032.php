...
class AdminMenu extends Model
{
    ...
    public static function getCurrentMenu($currentPath)
    {
        ...
        // 정확히 일치하는 메뉴 찾기 (backoffice 접두사 포함)
        ...
        // URL 경로의 일부가 일치하는 메뉴 찾기 (하위 메뉴의 경우)
        $pathParts = explode('/', trim($currentPath, '/'));
        
        if (count($pathParts) > 1) {
            $parentPath = $pathParts[0];
            $parentMenu = self::where('url', 'LIKE', '%' . $parentPath . '%')
                              ->where('is_active', true)
                              ->first();
        ...
        if (!empty($pathParts[0])) {
            ...
            $firstPathMenu = self::where('url', 'LIKE', '%' . $firstPath . '%')
                                 ->where('is_active', true)
                                 ->first();
            
            if ($firstPathMenu) {
                return $firstPathMenu;
            }
        }
