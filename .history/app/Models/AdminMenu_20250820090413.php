...
class AdminMenu extends Model
{
    ...
    public static function getCurrentMenu($currentPath)
    {
        ...
        // 3. /backoffice/ 접두사가 있는 경우 제거해서 찾기
        if (str_starts_with($currentPath, '/backoffice/')) {
            $exactMenuWithoutPrefix = self::where('url', substr($currentPath, 12)) // '/backoffice/' 길이만큼 제거
                                         ->where('is_active', true)
                                         ->first();
            
            if ($exactMenuWithoutPrefix) {
                return $exactMenuWithoutPrefix;
            }
        }

        // 4. 경로의 마지막 부분으로 메뉴 찾기 (가장 효과적)
        $pathParts = explode('/', trim($currentPath, '/'));
        if (!empty($pathParts)) {
            $lastPath = end($pathParts);
            $lastPathMenu = self::where('url', 'LIKE', '%' . $lastPath)
                                 ->where('is_active', true)
                                 ->orderBy('id') // 첫 번째 매칭되는 메뉴 선택
                                 ->first();
        ...
        // 5. URL에 포함된 경로로 메뉴 찾기 (백업)
        foreach (self::where('is_active', true)->orderBy('id')->get() as $menu) {
            if ($menu->url && str_contains($menu->url, $currentPath)) {
                return $menu;
            }
        }
