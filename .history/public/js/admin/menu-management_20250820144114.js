...
function initBackButtonHandler() {
    // 페이지가 뒤로가기로 로드되었는지 감지
    window.addEventListener('pageshow', function(event) {
        // event.persisted가 true면 뒤로가기로 온 것
        if (event.persisted) {
            // 뒤로가기로 돌아왔을 때 세션 메시지 DOM 제거
            ...
            // AJAX로 서버 세션도 제거
            fetch('/backoffice/menus/clear-session', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    // localStorage에 페이지 방문 표시
    if (!localStorage.getItem('menu_page_visited')) {
        localStorage.setItem('menu_page_visited', 'true');
    } else {
        // 이미 방문한 적이 있으면 뒤로가기일 가능성이 높음
        const successAlert = document.querySelector('.alert-success');
        if (successAlert) {
            successAlert.remove();
        }
...
function checkSessionMessage() {
    ...
    if (successAlert && successAlert.textContent.trim() !== '') {
        showAlertModal('성공', successAlert.textContent.trim(), 'success');
        
        // 세션 메시지를 제거하여 중복 표시 방지
        successAlert.remove();
        
        // localStorage 초기화 (새로운 작업 완료)
        localStorage.removeItem('menu_page_visited');
    }
}
