...
function checkSessionMessage() {
    // 성공 메시지가 있는지 확인 (data-show-modal="true" 속성이 있는 경우만)
    const successAlert = document.querySelector('.alert-success[data-show-modal="true"]');
    if (successAlert && successAlert.textContent.trim() !== '') {
        ...
        // AJAX로 세션에서 modal_message 제거 요청
        fetch('/backoffice/menus/clear-modal', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
        }).catch(error => console.error('Error clearing modal session:', error));
    }
}
