                ...
                <div class="board-card-body">
                    <form action="{{ route('backoffice.boards.update', $board) }}" method="POST">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-col form-col-6">
                                <div class="form-group">
                                    <label for="slug" class="form-label">게시판명 <span class="required">*</span></label>
                                    <input type="text" class="form-control" id="slug" name="slug" value="{{ old('slug', $board->slug) }}" readonly>
                                    <small class="form-text">게시판명은 수정할 수 없습니다.</small>
                                </div>
                            </div>
                        </div>
                                                ...
                                                <div class="board-form-row">
                                                    ...
                                                    <div class="board-form-col board-form-col-4">
                                                        <div class="board-form-group">
                                                            ...
                                                            <input type="number" class="board-form-control" name="custom_fields[{{ $index }}][max_length]" value="{{ $fieldConfig['max_length'] ?? '' }}" placeholder="예: 50" min="1" max="255" onchange="updateMaxLengthHelp(this, {{ $index }})">
                                                            <small class="board-form-text" id="max_length_help_{{ $index }}">{{ $fieldConfig['max_length'] ? '최대 ' . $fieldConfig['max_length'] . '자 (한글 기준)' : '한글 기준으로 계산됩니다' }}</small>
                                                        </div>
                                                    </div>
                                                    <div class="board-form-col board-form-col-4">
                                                        <div class="board-form-group">
...
<script>
            ...
            <div class="custom-field-item" id="${fieldId}">
                ...
                <div class="board-form-row">
                    ...
                    <div class="board-form-col board-form-col-4">
                        <div class="board-form-group">
                            <label class="board-form-label">최대 길이</label>
                            <input type="number" class="board-form-control" name="custom_fields[${existingFields}][max_length]" placeholder="예: 50" min="1" max="255" onchange="updateMaxLengthHelp(this, ${existingFields})">
                            <small class="board-form-text" id="max_length_help_${existingFields}">한글 기준으로 계산됩니다</small>
                        </div>
                    </div>
                    <div class="board-form-col board-form-col-4">
                        <div class="board-form-group">
    ...
    function updateMaxLengthHelp(input, index) {
        const helpText = document.getElementById(`max_length_help_${index}`);
        if (helpText) {
            const maxLength = input.value;
            if (maxLength > 0) {
                helpText.textContent = `최대 ${maxLength}자 (한글 기준)`;
            } else {
                helpText.textContent = '한글 기준으로 계산됩니다';
            }
        }
    }

    // 전역 함수로도 사용할 수 있도록 window 객체에 추가
    window.addCustomField = addCustomField;
    window.removeCustomField = removeCustomField;
    window.moveCustomField = moveCustomField;
    window.toggleFieldOptions = toggleFieldOptions;
    window.updateMaxLengthHelp = updateMaxLengthHelp;
</script>
@endpush
