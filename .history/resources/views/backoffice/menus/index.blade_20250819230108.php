    ...
    @forelse($menus as $menu)
        <li class="menu-item" data-id="{{ $menu->id }}">
            ...
            <div class="menu-actions">
                <a href="{{ route('backoffice.menus.edit', $menu) }}" class="btn btn-primary btn-table">수정</a>
                <form action="{{ route('backoffice.menus.destroy', $menu) }}" method="POST" class="delete-form" data-name="{{ $menu->name }}">
                    @csrf
                    @method('DELETE')
                    <button type="button" class="btn btn-danger btn-table delete-menu-btn">삭제</button>
                </form>
            </div>
        </li>
                ...
                <ul class="submenu-list" data-parent-id="{{ $menu->id }}">
                    @foreach($menu->children as $child)
                        <li class="submenu-item" data-id="{{ $child->id }}">
                            ...
                            <div class="menu-actions">
                                <a href="{{ route('backoffice.menus.edit', $child) }}" class="btn-edit">수정</a>
                                <form action="{{ route('backoffice.menus.destroy', $child) }}" method="POST" class="delete-form" data-name="{{ $child->name }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn-delete delete-menu-btn">삭제</button>
                                </form>
                            </div>
                        </li>
                    @endforeach
