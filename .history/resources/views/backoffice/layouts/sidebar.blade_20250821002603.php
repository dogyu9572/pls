            ...
            @php
                ...
                if($menu->url) {
                    ...
                    if(!empty($menuPath)) {
                        ...
                        // boards 메뉴는 /backoffice/boards와 정확히 일치할 때만 활성화
                        if ($menuPath === 'backoffice/boards') {
                            $isActive = $currentPath === 'backoffice/boards';
                        } else {
                            $isActive = $currentPath === $menuPath;
                        }
                        
                        // 디버깅용 로그 (나중에 제거 가능)
                        // \Log::info("Menu: {$menu->name}, Path: {$menuPath}, Current: {$currentPath}, Active: " . ($isActive ? 'true' : 'false'));
                    }
                }
