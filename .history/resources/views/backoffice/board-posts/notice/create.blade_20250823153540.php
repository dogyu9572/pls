    ...
    <div class="board-container">
        ...
        <div class="board-card">
            ...
            <div class="board-card-body">
                ...
                <form action="{{ route('backoffice.board-posts.store', $board->slug ?? 'notice') }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf

                    <div class="board-form-group">
                        <div class="board-checkbox-item">
                            <input type="checkbox" class="board-checkbox-input" id="is_notice" name="is_notice"
                                value="1" {{ old('is_notice') == '1' ? 'checked' : '' }}>
                    ...
                    <div class="board-form-group">
                        ...
                        <input type="text" class="board-form-control" id="title" name="title"
                        ...
                        @foreach ($board->custom_fields_config as $fieldConfig)
                            <div class="board-form-group">
                                ...
                                @elseif($fieldConfig['type'] === 'select')
                                    <select class="board-form-control" id="custom_field_{{ $fieldConfig['name'] }}"
                                        name="custom_field_{{ $fieldConfig['name'] }}"
                                        @if ($fieldConfig['required']) required @endif>
                                        ...
                                        @if ($fieldConfig['options'])
                                            @foreach (explode("\n", $fieldConfig['options']) as $option)
                                                ...
                                                @if (!empty($option))
                                                    <option value="{{ $option }}"
                                                        {{ old('custom_field_' . $fieldConfig['name']) == $option ? 'selected' : '' }}>
                                                        {{ $option }}
                                                    </option>
                                                @endif
                                            @endforeach
                                ...
                                @elseif($fieldConfig['type'] === 'checkbox')
                                                ...
                                                @if (!empty($option))
                                                    <div class="board-option-item">
                                                        <input type="checkbox"
                                                            ...
                                                            @if ($fieldConfig['required']) required @endif>
                                                        <label
                                                            for="option_{{ $fieldConfig['name'] }}_{{ $loop->index }}">{{ $option }}</label>
                                                    </div>
                                                @endif
                                            @endforeach
                                        </div>
                                    @else
                                        <div class="board-checkbox-item">
                                            <input type="checkbox" class="board-checkbox-input"
                                                id="custom_field_{{ $fieldConfig['name'] }}"
                                                name="custom_field_{{ $fieldConfig['name'] }}" value="1"
                                ...
                                @elseif($fieldConfig['type'] === 'date')
                                    <input type="date" class="board-form-control"
                                        id="custom_field_{{ $fieldConfig['name'] }}"
                                        name="custom_field_{{ $fieldConfig['name'] }}"
                                        value="{{ old('custom_field_' . $fieldConfig['name']) }}"
                                        @if ($fieldConfig['required']) required @endif>
                    ...
                    <div class="board-form-group">
                        ...
                        <div class="board-file-upload">
                            <div class="board-file-input-wrapper">
                                <input type="file" class="board-file-input" id="attachments" name="attachments[]"
