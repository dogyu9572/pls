        ...
        <div class="board-card-body">
            ...
            <form action="{{ route('backoffice.board-posts.update', [$board->slug ?? 'notice', $post->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                @if($board->custom_fields_config && count($board->custom_fields_config) > 0)
                    ...
                    @endphp
                    @if($customFields)
                        <div class="board-form-group" style="background: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
                            <h6 style="color: #666; margin-bottom: 10px;">디버깅: 저장된 커스텀 필드 값</h6>
                            <pre style="font-size: 12px; color: #333;">{{ json_encode($customFields, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                        </div>
                    @endif
                    ...
                    @foreach($board->custom_fields_config as $fieldConfig)
                        @php
                            ...
                            // 디버깅: 현재 필드 값 확인
                            // dd($fieldConfig['name'], $fieldValue, $customFields);
                        ...
                        <div class="board-form-group">
                                ...
                                <select class="board-form-control" 
                                    ...
                                    @if($fieldConfig['options'])
                                        @foreach(explode("\n", $fieldConfig['options']) as $option)
                                            ...
                                            @if(!empty($option))
                                                <option value="{{ $option }}" {{ old('custom_field_' . $fieldConfig['name'], $post->custom_fields[$fieldConfig['name']] ?? '') == $option ? 'selected' : '' }}>
                                                    {{ $option }}
                                                </option>
                                            @endif
                                        @endforeach
                            ...
                            @elseif($fieldConfig['type'] === 'checkbox')
                                @if($fieldConfig['options'])
                                    <!-- 디버깅: 체크박스 값 확인 -->
                                    <div style="background: #e7f3ff; padding: 5px; margin-bottom: 10px; border-radius: 3px; font-size: 12px;">
                                        <strong>디버깅:</strong> 필드명: {{ $fieldConfig['name'] }}, 저장값: {{ is_array($fieldValue) ? json_encode($fieldValue, JSON_UNESCAPED_UNICODE) : $fieldValue }}, 타입: {{ gettype($fieldValue) }}
                                    </div>
                                            ...
                                            @if(!empty($option))
                                                <div class="board-option-item">
                                                    <input type="checkbox" 
                                                           ...
                                                           name="custom_field_{{ $fieldConfig['name'] }}[]" 
                                                           value="{{ $option }}"
                                                           {{ (is_array($fieldValue) ? in_array($option, $fieldValue) : $fieldValue == $option) || (is_array(old('custom_field_' . $fieldConfig['name'])) ? in_array($option, old('custom_field_' . $fieldConfig['name'])) : old('custom_field_' . $fieldConfig['name']) == $option) ? 'checked' : '' }}
                                                           @if($fieldConfig['required']) required @endif>
                                ...
                                @else
                                    <div class="board-checkbox-item">
                                        <input type="checkbox" 
                                               ...
                                               name="custom_field_{{ $fieldConfig['name'] }}" 
                                               value="1"
                                               {{ (is_array($fieldValue) ? in_array('1', $fieldValue) : $fieldValue == '1' || $fieldValue == 1) || (is_array(old('custom_field_' . $fieldConfig['name'])) ? in_array('1', old('custom_field_' . $fieldConfig['name'])) : old('custom_field_' . $fieldConfig['name']) == '1') ? 'checked' : '' }}
                                               @if($fieldConfig['required']) required @endif>
                                        <label for="custom_field_{{ $fieldConfig['name'] }}" class="board-form-label">
                                            {{ $fieldConfig['label'] }}
                                        </label>
                            ...
                            @elseif($fieldConfig['type'] === 'radio')
                                @if($fieldConfig['options'])
                                    <!-- 디버깅: 라디오 버튼 값 확인 -->
                                    <div style="background: #fff3cd; padding: 5px; margin-bottom: 10px; border-radius: 3px; font-size: 12px;">
                                        <strong>디버깅:</strong> 필드명: {{ $fieldConfig['name'] }}, 저장값: {{ is_array($fieldValue) ? json_encode($fieldValue, JSON_UNESCAPED_UNICODE) : $fieldValue }}, 타입: {{ gettype($fieldValue) }}
                                    </div>
                            ...
                            @if($fieldConfig['max_length'] && in_array($fieldConfig['type'], ['text']))
                                <small class="board-form-text">최대 {{ $fieldConfig['max_length'] }}자까지 입력 가능합니다.</small>
                            @endif
                        </div>
                    @endforeach
                @endif
