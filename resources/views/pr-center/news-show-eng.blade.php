...
<div id="mainContent" class="container g{{ $gNum }} s{{ $sNum }} no_aside">
	<div class="inner">
		...
		<div class="board_view">
			...
			<div class="view_foot">
				@if($prevPost)
					<a href="{{ route('eng.pr-center.news.show', $prevPost->id) }}" class="arrow prev"><span>Previous</span>{{ $prevPost->title }}</a>
				@else
					<span class="arrow prev disabled"><span>Previous</span>No previous post.</span>
				@endif
				
				@if($nextPost)
					<a href="{{ route('eng.pr-center.news.show', $nextPost->id) }}" class="arrow next"><span>Next</span>{{ $nextPost->title }}</a>
				@else
					<span class="arrow next disabled"><span>Next</span>No next post.</span>
				@endif
				
				<a href="{{ route('eng.pr-center.news') }}" class="btn_list">List</a>
			</div>
		</div>
