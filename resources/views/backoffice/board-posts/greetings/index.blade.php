            ...
            <div class="board-card-body">
                ...
                @if($board->enable_sorting)
                    ...
                    <div class="sorting-controls">
                        <h6><i class="fas fa-sort"></i> 정렬 관리</h6>
                        <div class="sorting-info">
                            드래그 앤 드롭으로 순서를 변경하거나 숫자를 직접 입력하여 정렬할 수 있습니다.
                        </div>
                        <div class="sort-actions">
                            <button type="button" class="btn btn-sm btn-outline-primary" onclick="autoSetSortOrder()">
                                <i class="fas fa-magic"></i> 자동 정렬
                            </button>
                            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="resetSortOrder()">
                                <i class="fas fa-undo"></i> 초기화
                            </button>
                        </div>
                    </div>

                    <div class="sortable-list sorting-enabled">
                        @forelse($posts as $post)
                            <div class="post-item" data-post-id="{{ $post->id }}">
                                <div class="board-post-title">
                                    <i class="fas fa-grip-vertical sort-handle" title="드래그하여 순서 변경"></i>
                                    <span class="sort-order-display">{{ $post->sort_order ?? 0 }}</span>
                                    <input type="number" class="sort-input" value="{{ $post->sort_order ?? 0 }}" min="0" title="정렬 순서 입력">
                                    <span class="board-post-title-text">{{ $post->title }}</span>
                                    <div class="sort-actions">
                                        <a href="{{ route('backoffice.board-posts.edit', [$board->slug ?? 'notice', $post->id]) }}" class="btn btn-sm btn-primary">
                                            <i class="fas fa-edit"></i> 수정
                                        </a>
                                        <form action="{{ route('backoffice.board-posts.destroy', [$board->slug ?? 'notice', $post->id]) }}" method="POST" class="d-inline" onsubmit="return confirm('정말 삭제하시겠습니까?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <div class="text-center p-4">게시글이 없습니다.</div>
                        @endforelse
                    </div>
                @else
                    <!-- 정렬 기능이 비활성화된 경우 (기존 테이블 형태) -->
